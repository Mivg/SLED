...
def main():
    ...
    parser = CustomHfArgumentParser((ModelArguments, DataTrainingArguments, TrainingOverridesArguments))
    model_args, data_args, training_args = parser.parse_dictionary_and_args()
    ...
    if training_args.gradient_checkpointing and getattr(model.config, 'use_cache', False):
...
def _preprocess_tokenized_inputs():
    raise NotImplementedError('did not get here it')  # TODO - implement input_prefix support here as well
    input_ids = []
    attention_mask = []
